<?php
	...
	if(isset($_POST['name']) && isset($_POST['username']) && isset($_POST['userright']) && isset($_POST['dep'])) {
		...
		if(!preg_match('/^[a-zA-Z0-9][a-zA-Z0-9_]{1,15}$/i', $username)) {  
			$errors['username']="用户名只能包括字母（A-Z，a-z）、数字（0-9）及下划线（_），必须以字母或数字开头。最少2个字符，最多16个字符。";
		} else {
		...
		if(preg_match('/^\s*$/', $name)) $errors['name']="姓名不能为空。";

		if(preg_match('/\s/', $name)) $errors['name']="姓名中不能有空格。";
